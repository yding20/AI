...
#	accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
#
#	# print("TEST : ", test.eval({X: X_train, Y: Y_train}))
#	print ("Train Accuracy:", accuracy.eval({X: X_train, Y: Y_train}))
#	print ("Test Accuracy:", accuracy.eval({X: X_test, Y: Y_test}))
...
#
#
#X_train_orig =  [data.values[0][0:4]]
#
#for i in range(1, len(data.values)) : 
#	X_train_orig = np.concatenate((X_train_orig, [data.values[i][0:4]]), axis=0)
#
#X_train = X_train_orig.T/3
#print(X_train.shape)


## it seems it is an array
...
#one_hot_matrix = tf.one_hot(X_train_orig, C, 3)
#sess = tf.Session()
